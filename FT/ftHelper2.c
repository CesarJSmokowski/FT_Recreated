...
//int takeMeasurements(int argc, char** argv){
int takeMeasurements(int SAMPLES_PER_SEC, int SAMPLING_SECS, char* OutputFileName){
...
//-----
  ...
  printf("Entered takeMeasurements()\n");
  //-- open input file --
  ...
  */  
  printf("right before initializing taskHandle etc\n");
  int32       error = 0;
  printf("After error, before taskHandle\n");
  TaskHandle  taskHandle = 0;
  printf("After taskHandle, before samples_read_per_channel\n");
  int32       samples_read_per_channel;
  printf("After samples_read_per_channel, before data[]\n");
  float64     data[ARRAY_SIZE_IN_SAMPLES];
  printf("After data[], before errBuff[]\n");
  //char        errBuff[2048]={'\0'};

  printf("DAQ_TASK_NAME = %s\n", DAQ_TASK_NAME);
  printf("taskHandle = %d\n", taskHandle);
  //printf("Right before DAQmxCreateTask() Called");
  DAQmxErrChk (DAQmxCreateTask(DAQ_TASK_NAME, &taskHandle));
  printf("After Task created! ---> taskHandle = %d\n", taskHandle);
  //printf("Task created!\n");
  ...
  printf("After Voltage Chan created! ---> taskHandle = %d\n", taskHandle);
